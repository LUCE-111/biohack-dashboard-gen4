...
import { HomeIcon, ListIcon, PillIcon, SettingsIcon } from './Icons';
...
export function PrimaryNavigation({ view, onChange }: PrimaryNavigationProps) {
  return (
    <>
      <nav aria-label="주요 화면" className="mb-5 hidden rounded-2xl border border-white/8 bg-white/[0.025] p-1.5 md:grid md:grid-cols-4">
        {items.map((item) => {
      ...
      <nav
        aria-label="주요 화면"
        className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-4 border-t border-white/10 bg-[#0b0e14]/95 px-2 pb-[calc(env(safe-area-inset-bottom)+0.4rem)] pt-1.5 backdrop-blur-xl md:hidden"
      >
        {items.map((item) => {
          const active = item.view === view;
          return (
